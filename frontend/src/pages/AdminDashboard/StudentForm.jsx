...
const StudentForm = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      setCourses([
        { _id: 'jee', courseName: 'JEE', price: 25000 },
        { _id: 'neet', courseName: 'NEET', price: 30000 },
        { _id: 'class9-10', courseName: 'Class 9th & 10th', price: 15000 },
        { _id: 'class11-12', courseName: 'Class 11th & 12th', price: 20000 }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const loadStudentData = async () => {
    try {
      ...
      setFormData({
        studentPhoto: null, // Image is not re-loaded as file, only preview
        ...
        result: null // Result is not re-loaded as file, only preview
      });
      ...
      if (student.result?.url) {
        setResultPreview(student.result.url);
      }
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'course') {
      ...
      if (selectedCourse) {
        setFormData(prev => ({
          ...prev,
          course: value, // This will be the course ID
  ...
  const handleResultChange = (e) => {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onload = (e) => {
        setResultPreview(e.target.result);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const studentData = {
          ...formData,
          studentPhoto: photoPreview, // Send base64 image data
          result: resultPreview, // Send result image data
          dateOfBirth: new Date(formData.dateOfBirth).toISOString()
        };

        let response;
        if (isEditMode && studentId) {
          response = await studentAPI.update(studentId, studentData);
        ...
        navigate('/admin/students-management'); // Navigate back after success
      } catch (error) {
  ...
  if (loading && isEditMode) { // Show loading spinner when loading data in edit mode
    return (
