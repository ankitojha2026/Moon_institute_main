...
const StudentLogin = () => {
    ...
    // --- UPDATED: handleSubmit to use the new PHP login API ---
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);

    try {
        // formData mein 'studentName' aur 'password' hai
        const response = await studentAPI.login(formData);
        
        if (response.success) {
            // response.data mein poora student object aayega
            ...
            toast.error(response.message); // PHP se aaya hua error message dikhayein
        }
    } catch (error) {
